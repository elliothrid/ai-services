...
- `--interactive` и `--review-all` требуют TTY (проверка в `__main__`). Без неё батч
  из планировщика молча повиснет на первом вопросе. `ConsolePrompter` на EOF бросает
  `ParseAmbiguous`, `ask_yes_no` на EOF отвечает «нет» — закрытый пайп не должен
