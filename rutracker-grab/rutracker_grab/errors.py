...
class BrowserBusy(GrabError):
    """Профиль Chromium занят другим процессом rutracker-grab (§6a).

    Без этой проверки второй процесс не видит cookie и рапортует `NotLoggedIn` —
    пользователь идёт логиниться, хотя логин цел.
    """
